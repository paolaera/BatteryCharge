...
for i = 9000:10000
    if energy(i) < 0 %l'energia del fotovoltaico non è abbastanza
        ...
        while energy(i) < 0 & SOC(NumBattery,i)~=20 & j <= length(I)
            ...
            if j == size(maxCharge,1)    % usiamo questo per evitare che finisca su I(0)
        ...
        while energy(i) > 0 & SOC(NumBattery,i)~=100 & j <= length(I)
            [battery(NumBattery,i+1),energy(i)] = BatteryCharge(battery(NumBattery,i),energy(i),maxCharge(NumBattery),maxCharge15min(NumBattery));%applico la funzione scarica
            SOC(NumBattery,i+1) = SOCcontrol(battery(NumBattery,i+1),maxCharge(NumBattery));
            if j == size(maxCharge,1)    % usiamo questo per evitare che finisca su I(size(battery)+1)
                NumBattery = I(j); 
            else
                NumBattery = I(j+1);
            end
